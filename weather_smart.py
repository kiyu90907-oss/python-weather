        ...
        if r.status_code == 200:
            ...
            if "results" in data and len(data["results"]) > 0:
                ...
                return {
                    ...
                    "全名": 结果.get("name", 城市名)   # 取官方地名，取不到就用用户输入的
                }
...
# 保存天气函数 这个函数和22几乎一样 只是在保存内容里多了天气描述
def 保存天气(城市名, 数据):
    """将天气数据追加写入本地日志文件"""
...
# Python程序的标准入口
# 如果是直接运行这个文件而不是被别的文件导入 就执行main()函数
